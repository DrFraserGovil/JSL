...
{
	bool hasDash = (nextElement[0] ==
					'-'); // dashes signify commands, but also negative nos.
	bool isSingleCharacter = (std::strlen(nextElement) == 1);

	if (!hasDash || isSingleCharacter)
	{
		return false; // entries that are not preceeded by a dash, or are a single
					  // character long, cannot be command triggers
	}

	return !isdigit(nextElement[1]); // negative numbers are not flags, even
									 // though they start with a dash.
}
...
namespace JSL::internal::Parameter
{
	...
	bool CommandLineReader::IsConfigured() { return Configured; }
	...
	void CommandLineReader::Parse(int argc, char **argv)
	{
		...
		while (idx < argc)
		{
			...
			if (arg.starts_with("-"))
			{
				...
				{
					Options[static_cast<std::string>(narg)] =
						"__bool_tag__"; // a flag with no value is treated as a boolean true
					++idx;
				}
			}
			else
	...
	void CommandLineReader::Configure(std::filesystem::path configFile,
		std::string_view configDelimiter)
	{
		IO::forSplitLineIn(configFile, configDelimiter, [&](auto linevec) {
			if (linevec.size() >= 2)
			{
				// treat the first element as the key, and the rest of the line as the
				// value, rejoining with the configDelimiter in case the value itself
				// contained the delimiter
				Options[static_cast<std::string>(Normalize(linevec[0]))] =
					JSL::String::stitch(linevec, 1, linevec.size(), configDelimiter);
			}
		});
	}
