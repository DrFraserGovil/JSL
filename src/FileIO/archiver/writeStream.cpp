...
namespace JSL::Archiver{
    ...
    internal::WriteExporter WriteStream::Export(const internal::Security & security)
    {
    ...
    void WriteStream::SetLarge(std::ostream & diskStream){
        IsLarge = true;
    ...
    bool WriteStream::NeedsWriting()
    {
        return !IsLarge;
    }

    
    WriteStream::WriteStream(std::string_view name) : Name(std::string(name)){
