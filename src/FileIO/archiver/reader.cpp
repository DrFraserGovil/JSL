...
namespace JSL::Archiver
{
	...
	bool Vault<Mode::Read>::ReadBlock(char* buffer)
	{
	...
	void  Vault<Mode::Read>::BuildIndex()
	{
	...
	Vault<Mode::Read>::Vault(): internal::VaultBase(Mode::Read)
	{
		Initialised = false;
	}
	...
	Vault<Mode::Read>::Vault(const std::string & archivePath, bool strictMode)  : internal::VaultBase(Mode::Read)
	{
		StrictMode = strictMode;
		Open(archivePath);
	}

	Vault<Mode::Read>::~Vault() noexcept
	{
		Close();
	}

	void Vault<Mode::Read>::Open(std::string_view path)
	{
	...
	void Vault<Mode::Read>::Close()
	{
		//Closing doesn't really do much beside shut off the file streams
		FileStream.close();
	}

	std::vector<std::string> Vault<Mode::Read>::ListFiles()
	{
	...
	ReadStream & Vault<Mode::Read>::operator[](const std::string & file)
	{
	...
	void Vault<Mode::Read>::ForLineIn(const std::string & file,std::function<void(std::string_view)> perLineFunction)
	{
