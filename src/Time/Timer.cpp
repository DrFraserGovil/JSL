...
namespace JSL::Time
{
	...
	double Timer::Lap()
	{
		...
		return duration_cast<nanoseconds>(std::chrono::high_resolution_clock::now() - start_time).count() / 1e9;
	}
	...
	double Timer::Measure()
	{
		...
		using namespace std::chrono;
		return duration_cast<nanoseconds>(stop_time - start_time).count() / 1e9;
	}
