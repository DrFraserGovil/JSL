...
TEST_CASE("FormatDuration basic formatting", "[time][string]")
{
    ...
    {
        REQUIRE(JSL::Time::FormatDuration(0.00004) == "[Time less than 0.05ms]");
        REQUIRE(JSL::Time::FormatDuration(0.0) == "[Time less than 0.05ms]");
    }

    SECTION("Negative values")
    {
        REQUIRE(JSL::Time::FormatDuration(-60.0) == "Negative 1 Minute");
        REQUIRE(JSL::Time::FormatDuration(-1.5) == "Negative 1.5 Seconds");
    }

    SECTION("Pluralization and Singular units")
    {
        REQUIRE(JSL::Time::FormatDuration(1.0) == "1 Second");
        REQUIRE(JSL::Time::FormatDuration(2.0) == "2 Seconds");
        REQUIRE(JSL::Time::FormatDuration(60.0) == "1 Minute");
        REQUIRE(JSL::Time::FormatDuration(120.0) == "2 Minutes");
    }
}
...
TEST_CASE("FormatDuration contrast and rounding", "[time][string]")
{
    SECTION("One-unit contrast logic")
    {
        ...
        REQUIRE(JSL::Time::FormatDuration(90061.0) == "1 Day 1 Hour");
        
        // 3661s = 1 Hour, 1 Minute, 1 Second
        // Should only show Hour + Minute
        REQUIRE(JSL::Time::FormatDuration(3661.0) == "1 Hour 1 Minute");
    }

    SECTION("Decimal tail and carry-over rounding")
    {
        // Check standard decimal
        REQUIRE(JSL::Time::FormatDuration(61.5) == "1 Minute 1.5 Seconds");

        // Check the 0.96 -> 1.0 carry over logic
        // 0.96 rounds nonIntegerTail to 10, which should increment integerTail
        REQUIRE(JSL::Time::FormatDuration(0.96) == "1 Second");
    }

    SECTION("High resolution threshold (< 0.1s)")
    {
        // Below 0.1s, the code should switch to Milliseconds
        REQUIRE(JSL::Time::FormatDuration(0.05) == "50 Milliseconds");
        REQUIRE(JSL::Time::FormatDuration(0.0055) == "5.5 Milliseconds");
    }
}
...
TEST_CASE("FormatDuration edge cases", "[time][string]")
{
    SECTION("Trailing space removal")
    {
        ...
        std::string result = JSL::Time::FormatDuration(3600.0);
    ...
    SECTION("Large durations")
    {
        ...
        REQUIRE(JSL::Time::FormatDuration(oneYearOneDay) == "1 Year 1 Day");
    }
