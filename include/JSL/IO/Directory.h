...
namespace JSL::IO
{
	struct FileMetadata
	{
		...
		uintmax_t Size;
		auto operator<=>(const FileMetadata &other) const
		{
	...
	class Directory
	{
	  public:
		...
		*/
		...
		static std::set<std::filesystem::path> match(std::filesystem::path trget, std::string matchPattern, bool recursive = false);
